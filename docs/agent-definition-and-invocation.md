...
```
task overlay > policy > platform > spec > base
```
...
- 공통 spec에 프레임워크 구현 세부사항을 넣지 않습니다.
...
재사용 가능한 기능 단위입니다. 하나의 spec은 하나의 기능에만 집중합니다.
...
`requires` 에 나열된 spec은 자동으로 함께 로드됩니다.
`conflicts_with` 에 나열된 spec과는 함께 사용할 수 없습니다.
...
| `policy` 가 `spec` 과 충돌 | `policy` 우선 |
| 두 spec이 서로 `conflicts_with` | 나중에 로드된 spec 제거, 경고 출력 |
...
| 충돌 우선순위 | task overlay > policy > platform > spec > base |
| policy 범위 | 프로젝트 전체에 공통 적용, 다른 프로젝트는 별도 policy |
| run 기록 | 실행 이력과 디버깅 근거로 유지 |
| spec 세부사항 | 프레임워크 구현 세부사항은 platform 모듈에 기술 |
