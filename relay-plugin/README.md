...
6. [context-compressor — 컨텍스트 압축 시스템](#context-compressor--컨텍스트-압축-시스템)
7. [사용 방법](#사용-방법)
8. [팀 구조 시각화](#팀-구조-시각화)
9. [스킬 레퍼런스](#스킬-레퍼런스)
10. [회의록](#회의록)
11. [데이터 저장 구조](#데이터-저장-구조)
12. [플러그인 구조](#플러그인-구조)
13. [제한사항](#제한사항)
...
```
base + capabilities + platform + policy + task → 최종 런타임 인스트럭션
```
...
│   ├── agent-profiles/          # MCP 호출 전용 프로파일 (teammate 모드와 분리)
...
> | 위치 | 용도 | 실행 방식 |
> |---|---|---|
> | `agents/` | Claude Code teammate 모드 | `ANTHROPIC_BASE_URL` 환경변수 오버라이드 |
> | `docs/agent-profiles/` | invoke-agent MCP 호출 참조 | `gemini_mcp` / `codex_mcp` 도구 호출 |
>
> 두 디렉토리를 혼용하지 마십시오. Gemini·OpenAI는 MCP 전용이므로 `agents/` 에 파일이 없습니다.
