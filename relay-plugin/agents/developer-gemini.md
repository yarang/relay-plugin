...
description: Gemini 기반 개발자. Claude teammate 로 실행되며 gemini_mcp 를 통해 Gemini 모델에 작업을 위임한다.
model: sonnet
effort: normal
platform: gemini
---
...
Claude Code teammate 모드로 실행되며, 실제 작업은 `gemini_mcp` 를 통해 Gemini 모델에 위임합니다.

> **Zai 와의 차이**: Zai teammate 는 `ANTHROPIC_BASE_URL` 교체로 모델 자체가 GLM 으로 바뀝니다.
> Gemini teammate 는 Claude Code 가 정상 실행되면서 MCP 도구(`gemini_mcp`)를 호출합니다.
...
사전 조건: `.mcp.json` 에 `gemini_mcp` 등록 필요 → `/relay:setup-keys`
...
gemini_mcp.gemini_generate(
  model   = "gemini-2.5-flash",   # expert 파일의 backed_by 에서 파싱
  system  = <1단계 합성 결과>,
...
모델 오버라이드: 리더가 `model: gemini-2.5-pro` 를 지정하면 해당 모델로 교체합니다.
